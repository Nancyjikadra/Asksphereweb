...
api_app.add_middleware(
    CORSMiddleware,
    allow_origins=[os.getenv("FRONTEND_URL")],  # Use environment variable for security
...
class VideoQAPipeline:
    ...
    def __init__(self, video_folder="videos", cache_dir="video_cache"):
        ...
        # Preprocess and cache all videos in the folder
...
async def predict(request: Request):
    ...
    result = await run_in_threadpool(pipeline.answer_question, question)
    return JSONResponse(result)

# Run FastAPI app
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(api_app, host="0.0.0.0", port=int(os.getenv("PORT", 8000)))
